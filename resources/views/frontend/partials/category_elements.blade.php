            ...
            <ul class="list-unstyled mb-3">
                ...
                @foreach (\App\Utility\CategoryUtility::get_immediate_children_ids($first_level_id) as $key => $second_level_id)
                    <li class="mb-2 fs-14 pl-2">
                        <a class="text-reset hov-text-primary" href="{{ route('products.category', get_single_category($second_level_id)->slug) }}">{{ get_single_category($second_level_id)->getTranslation('name') }}</a>
                    </li>
                @endforeach
            </ul>
        </div>
